...
class IntervalShake implements IFlxDestroyable
{
  ...
  function shakeProgress(timer:FlxTimer):Void
  {
    ...
    if (axes == FlxAxes.X || axes == FlxAxes.XY) object.x = initialOffset.x + FlxG.random.float((-curIntensity) * object.width, (curIntensity) * object.width);
    if (axes == FlxAxes.Y || axes == FlxAxes.XY) object.y = initialOffset.y + FlxG.random.float((-curIntensity) * object.width, (curIntensity) * object.width);
