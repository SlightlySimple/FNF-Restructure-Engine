...
class BaseGameConverter
{
	public static function convertCharacter()
	{
		...
		vbox.add(new Label("Old character sprite sheet folder (ex. \"shared/images\"):"));
		...
		var imageBrowse:Button = new Button(0, 0, "buttonLoad", function() {
			...
			file.loadCallback = function(fullPath:String) {
				...
				{
					while (nameArray[nameArray.length - 1] != "images")
						nameArray.pop();
		...
		vbox.add(new Label("New character sprite sheet folder (ex. \"modNameHere/images\"):"));
			...
			file.saveCallback = function(fullPath:String) {
				...
				if (nameArray.contains("images"))
				{
					...
					if (nameArray.contains("characters"))
						new Notify("WARNING: The file path should not contain a \"characters\" folder. Choose the folder that the \"characters\" folder is in");
				}
				else
					new Notify("The file path does not contain an \"images\" folder");
			}
			...
			if (FileSystem.exists(jsonInput.text))
			{
				...
				if (imagePathArray.contains("images"))
				{
					...
					if (FileSystem.exists(trueImagePath + "/" + character.assetPath + ".xml") || FileSystem.exists(trueImagePath + "/" + character.assetPath + ".txt"))
					{
						...
						var assetList:Array<String> = [character.assetPath];
						var assetPrefixSplit:Array<String> = finalImageInput.text.replace('\\','/').split('/');
						while (assetPrefixSplit[0] != "images")
							assetPrefixSplit.shift();
						assetPrefixSplit.shift();
						if (assetPrefixSplit.length > 0)
							assetPrefix = assetPrefixSplit.join("/") + "/";
						...
						var finalChar:CharacterData = {
							fixes: 1,
							asset: assetPrefix + character.assetPath,
							position: [210, 765],
							camPosition: [150, -100],
							scale: [1, 1],
							antialias: true,
						...
						for (a in oldAnims)
						{
							...
							if (a.assetPath != null && a.assetPath != character.assetPath)
							{
								anim.asset = assetPrefix + a.assetPath;
								if (!assetList.contains(a.assetPath))
									assetList.push(a.assetPath);
							}
							if (a.prefix != null)
						...
						for (asset in assetList)
						{
							...
							{
								var assetDirSplit:Array<String> = StringTools.replace(asset, "\\", "/").split("/");
								assetDirSplit.pop();
							...
							if (FileSystem.exists(trueImagePath + "/" + asset + ".png"))
								File.copy(trueImagePath + "/" + asset + ".png", assetPastePath + "/" + asset + ".png");
							if (FileSystem.exists(trueImagePath + "/" + asset + ".xml"))
								File.copy(trueImagePath + "/" + asset + ".xml", assetPastePath + "/" + asset + ".xml");
							if (FileSystem.exists(trueImagePath + "/" + asset + ".txt"))
								File.copy(trueImagePath + "/" + asset + ".txt", assetPastePath + "/" + asset + ".txt");
						}

						new Notify("Successfully converted the character \"" + convertedCharacterId.replace(".json", "") + "\" from the base game");
					}
				}
			}
		});
		...
		var convert:TextButton = new TextButton(0, 0, "Convert", function() {
			...
			if (FileSystem.exists(jsonInput.text))
			{
				...
				for (p in oldPieces)
				{
					...
					if (finalAssetPath != "" && stagePiece.asset.replace("\\", "/").startsWith(finalAssetPath))
				...
				new Notify("Successfully converted the stage \"" + convertedStageId.replace(".json", "") + "\" from the base game");
			}
		});
		btnHbox.add(convert);
	...
	public static function convertChart(callback:Void->Void)
	{
		...
		vbox.add(new Label("Old chart .json (ex. \"data/songs/songname/songname-chart.json\"):"));
		var jsonHBox:HBox = new HBox();
		var jsonInput:InputText = new InputText(0, 0, "");
		jsonHBox.add(jsonInput);
		var jsonBrowse:Button = new Button(0, 0, "buttonLoad", function() {
			...
			file.loadCallback = function(fullPath:String) {
				if (fullPath.indexOf("-chart") > -1 && FileSystem.exists(fullPath.replace("-chart", "-metadata")))
					...
					new Notify("The file does not end in \"-chart\" or the \"-metadata\" file could not be located");
			}
			file.load("json");
		});
		jsonHBox.add(jsonBrowse);
		...
		vbox.add(new Label("Old chart music folder (ex. \"songs/songname\"):"));
		var musicHBox:HBox = new HBox();
		var musicInput:InputText = new InputText(0, 0, "");
		musicHBox.add(musicInput);
		var musicBrowse:Button = new Button(0, 0, "buttonLoad", function() {
			var file:FileBrowser = new FileBrowser();
			file.loadCallback = function(fullPath:String) {
				var nameArray:Array<String> = fullPath.replace('\\','/').split('/');
				nameArray.pop();
		...
		var convert:TextButton = new TextButton(0, 0, "Convert", function() {
			if (jsonInput.text.indexOf("-chart") > -1 && FileSystem.exists(jsonInput.text.replace("-chart", "-metadata")))
			{
				var pathArray:Array<String> = jsonInput.text.replace('\\','/').split('/');
				var convertedSongId:String = pathArray[pathArray.length - 1].split("-chart")[0];

				var chart:Dynamic = Json.parse(File.getContent(jsonInput.text));
				var metadata:Dynamic = Json.parse(File.getContent(jsonInput.text.replace("-chart", "-metadata")));
				...
				var trueMusicPath:String = musicInput.text + "/";
				...
				for (t in tracks)
				{
					if (FileSystem.exists(trueMusicPath + t[0] + ".ogg") && !FileSystem.exists(trueSavePath + "/" + t[0] + ".ogg"))
						File.copy(trueMusicPath + t[0] + ".ogg", trueSavePath + "/" + t[0] + ".ogg");
				}
				...
				new Notify("Successfully converted the song \"" + convertedSongId + "\" from the base game");
			}
			else
				new Notify("The file does not end in \"-chart\" or the \"-metadata\" file could not be located");
		});
